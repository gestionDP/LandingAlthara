...
/**
 * Portal del REVISOR (abogado/fiscal) — doble visado de documentos.
 * Muestra la cola de documentos con algún visado pendiente para los roles del
 * usuario (puede tener ambos: abogado y fiscal). Por cada rol pendiente ofrece
 * aprobar / rechazar (con motivo), con vista previa incrustada y una barra de
 * progreso que refleja el estado combinado. Un documento no llega al inversor
 * hasta que el visado legal y el fiscal están ambos aprobados.
 */
...
const ROLE_LABELS: Record<Role, string> = { legal: 'Abogado', tax: 'Fiscal' };
...
export default function ReviewPage() {
  ...
  async function approve(item: QueueItem, role: Role) {
    ...
    const res = await fetchJson(`/api/dataroom/review/documents/${item.id}`, {
      method: 'POST',
      body: JSON.stringify({ role, decision: 'approve' }),
    });
    setActionBusy(null);
    if (res.ok) { toast(`Visado ${ROLE_LABELS[role].toLowerCase()} aprobado.`, 'ok'); load(); }
    else toast('No se ha podido aprobar el documento.', 'error');
  }

  async function confirmReject(item: QueueItem, role: Role) {
    ...
    const res = await fetchJson(`/api/dataroom/review/documents/${item.id}`, {
      method: 'POST',
      body: JSON.stringify({ role, decision: 'reject', reason: reason.trim() }),
    });
    setActionBusy(null);
    if (res.ok) { toast(`Visado ${ROLE_LABELS[role].toLowerCase()} rechazado.`, 'ok'); setRejecting(null); setReason(''); load(); }
  ...
  const rolesLabel = data.roles.map((r) => ROLE_LABELS[r]).join(' y ');
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-[#1c3742]/60">
          Revise cada documento y apruebe o rechace su visado. Un documento no se publica al inversor
          hasta que el visado del abogado y el del fiscal están ambos aprobados.
        </p>
      </div>
      ...
      {data.items.length === 0 ? (
        ...
        <ul className="space-y-3">
          {data.items.map((item) => {
            const pendingRoles = data.roles.filter(
              (r) => (r === 'legal' ? item.legalStatus : item.taxStatus) === 'pending',
            );
            return (
              <li key={item.id} className="rounded-lg border border-[#1c3742]/10 bg-white p-4">
                ...
                {/* Barra de progreso del doble visado */}
                <div className="mt-4 max-w-md">
                  <VisadoProgress legalStatus={item.legalStatus} taxStatus={item.taxStatus} />
                </div>
                {(item.legalReason || item.taxReason) && (
                  <div className="mt-2 space-y-1">
                    {item.legalStatus === 'rejected' && item.legalReason && (
                      <p className="text-[11px] text-red-600">Motivo (abogado): {item.legalReason}</p>
                    )}
                    {item.taxStatus === 'rejected' && item.taxReason && (
                      <p className="text-[11px] text-red-600">Motivo (fiscal): {item.taxReason}</p>
                    )}
                  </div>
                )}

                {/* Acciones por cada rol pendiente del usuario */}
                    ...
                    return (
                      <div key={role}>
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="text-[11px] font-medium uppercase tracking-wider text-[#1c3742]/55">
                            Como {ROLE_LABELS[role]}
                          </span>
                          <button
                            onClick={() => approve(item, role)}
                          ...
                          <button
                            onClick={() => { setRejecting(key); setReason(''); }}
                            disabled={busy || isRejecting}
                        ...
                        {isRejecting && (
                          <div className="mt-3">
                            <label className="mb-1 block text-[11px] font-medium uppercase tracking-wider text-[#1c3742]/60">
                              Motivo del rechazo ({ROLE_LABELS[role]}) *
                            </label>
                            <textarea
                              value={reason}
                            ...
                            <div className="mt-2 flex gap-2">
                              <button
                                onClick={() => confirmReject(item, role)}
