...
export async function createCategory(projectId: string, name: string, level = 2) {
  ...
  const [cat] = await db().insert(schema.documentCategories).values({
    tenant: T, projectId, name: clean, slug, sortOrder: existing.length, level: level === 1 ? 1 : 2,
  }).returning();
  return cat;
}
...
/** Elimina la carpeta; sus documentos pasan a la raíz (sin carpeta). */
